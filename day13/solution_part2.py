...
def parse_input(filename, offset=0):
    machines = []
    ...
    while i < len(lines):
        ...
        p_line = lines[i + 2]
        px = int(p_line.split('X=')[1].split(',')[0]) + offset
        py = int(p_line.split('Y=')[1]) + offset
...
def solve_machine(machine, debug=False):
    ...
    # Check constraints: non-negative only (no 100-press limit for part 2)
    if a < 0 or b < 0:
        if debug:
...
def main():
    parser = argparse.ArgumentParser(description='Day 13: Claw Contraption Part 2')
    ...
    offset = 10000000000000  # Part 2 offset
    machines = parse_input(filename, offset)
    
    if args.debug or args.test:
        print(f"Loaded {len(machines)} machines from {filename} with offset {offset}")
